...
| POST | `/v1/customer-case-management-plan/initiate` | Initiate |
| GET | `/v1/customer-case-management-plan` | Retrieve (list) |
| GET | `/v1/customer-case-management-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/customer-case-management-plan/{crId}/update` | Update |
| PUT | `/v1/customer-case-management-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/customer-case-management-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-customer-case-management:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-customer-case-management ./helm -n bian-sales-service
```

Exposed through the platform Gateway at path prefix `/sd-customer-case-management` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
